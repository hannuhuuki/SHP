function [simul_hourly_flow, simul_hourly_spilling, simul_flow_dispatch, ...
    simul_flow_dispatch_correction, simul_revenue, balancing_fine, simul_water_level, simul_spilling] = ...
...
%%%%%%% Use the Q-table for inflow years 2000 to 2017 flow dispatch optimization %%%%%%%
...
d = 18*365; % 2000 - 2017 is 18 years
...
for day = 1:d
    ...
    if next_water_level > stateWaterHeight(end) 
        ...
        simul_hourly_flow((1+(day-1)*hours_in_day):(hours_in_day+(day-1)*hours_in_day)) = optimFlow_2018(:,day_ind(day),action_ind);
    ...
    % (buy the missing energy at expected up-regulating price)
        ...
        % Hourly flow (m^3/s) for each hour of day 
        simul_hourly_flow((1+(day-1)*hours_in_day):(hours_in_day+(day-1)*hours_in_day)) = optimFlow_2018(:,day_ind(day),corrected_action_ind);
